...
#        ALL_PEAKS.append("08peak_macs1/{}_vs_{}_macs1_nomodel_peaks.bed".format(case, control))
	...
	ALL_HMCAN_PEAKS.append("hmcan/{}_vs_{}_peaks.narrowPeak".format(case, control))
	ALL_HMCAN_CNV.append("hmcan/{}_vs_{}_CNV_profile.txt".format(case, control))
	ALL_HMCAN_CNV_PNG.append("hmcan/{}_vs_{}_CNV_profile.txt.png".format(case, control))
	ALL_HMCAN_BW.append("hmcan/{}_vs_{}.bw".format(case, control))
	ALL_HMCAN_RENORM.append("hmcan/{}_vs_{}.renorm.bw".format(case, control))
	ALL_LILY.append("lily/{}_vs_{}.scores.bed".format(case, control))
...
TARGETS.extend(ALL_HMCAN_PEAKS)
TARGETS.extend(ALL_HMCAN_CNV)
TARGETS.extend(ALL_HMCAN_CNV_PNG)
TARGETS.extend(ALL_HMCAN_BW)
TARGETS.extend(ALL_HMCAN_RENORM)
TARGETS.extend(ALL_LILY)
...
rule down_sample:
    ...
    run:
        ...
        shell("sambamba view -f bam -t 5 --subsampling-seed=3 -s {rate} {inbam} | samtools sort -m 2G -@ 16 -T {outbam}.tmp > {outbam} 2> {log}".format(rate = down_rate, inbam = input[0], outbam = output[0], log = log))
...
rule make_inputSubtract_bigwigs:
    ...
    shell:
        """
        bamCompare --bamfile1 {input[1]} --bamfile2 {input[0]} --normalizeUsing RPKM --operation subtract --binSize 30 --smoothLength 300 -p 16  --extendReads 200 -o {output} 2> {log}
...
rule superEnhancer:
    input : "04aln_downsample/{control}-downsample.sorted.bam", "04aln_downsample/{case}-downsample.sorted.bam",
            "04aln_downsample/{control}-downsample.sorted.bam.bai", "04aln_downsample/{case}-downsample.sorted.bam.bai","09peak_macs2/{case}_vs_{control}_macs2_peaks.gappedPeak"
...
if config["LILY"]:
	rule call_hmcan:
		input: "04aln_downsample/{case}-downsample.sorted.bam", "04aln_downsample/{control}-downsample.sorted.bam"
		output: "hmcan/{case}_vs_{control}_peaks.narrowPeak","hmcan/{case}_vs_{control}_CNV_profile.txt"
		log: "00log/hmcan_{case}.log"
		conda: "envs/lily.yaml"
		shell:
			"""
			mkdir -p hmcan
			{config[lily][hmcan-exe]} {input[0]} {input[1]} {config[lily][hmcan-config]} {config[prefix]}/hmcan/{wildcards.case}_vs_{wildcards.control}
			"""
	
	rule call_lily:
		input: "hmcan/{case}_vs_{control}_peaks.narrowPeak"
		output: "lily/{case}_vs_{control}.scores.bed"
		conda: "envs/lily.yaml"
		log: "logs/lily_{case}.log"
		shell:
			"""
			mkdir -p lily
			cat {config[lily][lily-R]} | R --slave --args {config[prefix]}/hmcan/{wildcards.case}_vs_{wildcards.control} lily/ 12500 2500 {config[lily][refseq]} {config[lily][chrom_sizes]} {log}
			"""
	
	rule print_cnv:
		input: "hmcan/{case}_vs_{control}_CNV_profile.txt"
		output: "hmcan/{case}_vs_{control}_CNV_profile.txt.png"
		conda: "envs/lily.yaml"
		shell:
			"""
			cat {config[lily][freec]} | R --slave --args 2 {input}
			"""
	
	rule compute_scaling_factors:
		input: peaks=ALL_HMCAN_PEAKS,
			bigwigs=ALL_HMCAN_BW
		output: table="renorm-factors.txt"
		params: renorm=ALL_HMCAN_RENORM
		conda: "envs/lily.yaml"
		script: "scripts/compute-scaling-factors.R"
	
	
	rule re_normalize:
		input: bigwig="hmcan/{case}_vs_{control}.bw", table="renorm-factors.txt" 
		output: renorm="hmcan/{case}_vs_{control}.renorm.bw"
		conda: "envs/lily.yaml"
		script: "scripts/renormalizeWig.R"
